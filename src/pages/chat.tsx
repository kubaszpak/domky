...
} from "@/server/trpc/router/message";
...
const Chat: NextPage<{ chats: string }> = ({ chats }) => {
	...
	const [parsedChats, setParsedChats] = useState<fetchUserReturnType>(
		JSON.parse(chats)
	);
	const [selectedChat, setSelectedChat] = useState<
		fetchUserReturnType[number] | null | undefined
	>(null);
	const {
		status: queryStatus,
	...
	useEffect(() => {
		if (parsedChats && parsedChats.length > 0) {
			setSelectedChat((prev) =>
				!!prev
					? parsedChats.find((chat) => chat.id === prev.id)
					: parsedChats[0]
			);
		}
	}, [parsedChats]);
